...
<p style="text-indent: 50px; line-height: 1.5;">Sebagai upaya pengembangan kompetensi SDM bidang ke PUPR-an dan menindaklanjuti Surat Kepala Pusat Pengembangan Kompetensi Sumber Daya Air dan Permukiman Nomor SM 0304-Ma/107 tanggal 01 Februari 2024 Hal Penetapan Calon Peserta Pelatihan Integrated Coastal Lowland Development, dengan hormat kami mohon Bapak/Ibu berkenan untuk menugaskan pegawai yang namanya tercantum di bawah ini untuk mengikuti Pelatihan {{ $pelatihan->nama }} yang akan dilaksanakan pada tanggal {{ rentang_tgl($pelatihan->tanggal_mulai, $pelatihan->tanggal_selesai) }} secara {{ $pelatihan->model_pelatihan->nama }}. Adapun ketentuan yang harus dipenuhi oleh calon peserta pelatihan adalah sebagai berikut </p>
  ...
  {{-- Lampiran 1 --}}
  <p style="text-align: right; page-break-before: always;">Lampiran I Surat Kepala Balai Pengembangan Kompetensi PUPR Wilayah VI Surabaya</p>
  ...
  <ol>
    ...
    {{-- Lampiran 2 --}}
  <p style="text-align: right; page-break-before: always;">Lampiran II Surat Kepala Balai Pengembangan Kompetensi PUPR Wilayah VI Surabaya</p>
  <table style="text-align: right">
      ...
      @foreach($data as $item)
        ...
        <thead>
          <tr style="page-break-inside: avoid;">
            <td style="background-color:aqua;padding:10px;">{{ $item['no'] }}</td>
            <td style="background-color:aqua;text-align:center;padding:10px;">{{ $item['nama'] }}</td>
            <td style="background-color:aqua;text-align:center;padding:10px;">{{ $item['nip'] }}</td>
            <td style="background-color:aqua;text-align:center;padding:10px;">{{ $item['jabatan'] }}</td>
            <td style="background-color:aqua;text-align:center;padding:10px;">{{ $item['unit_kerja'] }}</td>
            <td style="background-color:aqua;text-align:center;padding:10px;">{{ $item['keterangan'] }}</td>
          </tr>
        </thead>
        <tbody>
        @else
        <tr style="page-break-inside: avoid;">
          ...
          <td style="text-align:center;padding:10px;vertical-align:middle">{{ $item['nip'] }}</td>
          ...
          @if (!$item['keterangan'] == null)
            <td id="keterangan_{{ $temp }}" rowspan="1" style="text-align:center;padding:10px;vertical-align:middle">{{ $item['keterangan'] }}</td>
            @php
              $temp++;
            @endphp
          @else
            <script>
              document.getElementById('keterangan_{{ $temp-1 }}').rowSpan += 1;
            </script>
          @endif
        </tr>
        @endif
    ...
    {{-- <p style="text-align: right;page-break-before: always;">Lampiran III Surat Kepala Balai Pengembangan Kompetensi PUPR Wilayah VI Surabaya</p>
    <table style="text-align: right">
        <tr>
            <td>Nomor</td>
            <td>:</td>
    ...
    <div style="text-align: center;">
        <hr class="text-bold" style="width: 50%;font-weight:bold;">
        <div style="position: relative; top:-20px; background-color: white; display: inline-block; padding: 0 10px;">KOP INSTANSI</div>
    </div>
    ...
    <h3 style="text-align:center;page-break-before: always;">
        ...
        PESERTA PELATIHAN {{ $pelatihan->nama }}
        <br>
        TANGGAL {{ rentang_tgl($pelatihan->tanggal_mulai, $pelatihan->tanggal_selesai) }}
    </h3>
    ...
    <p class="text-indent:50px;line-height:1.5">
        Menyatakan bahwa Saya bersedia mengikuti Peserta Pelatihan {{ $pelatihan->nama }} pada tanggal {{ rentang_tgl($pelatihan->tanggal_mulai, $pelatihan->tanggal_selesai) }} secara {{ $pelatihan->model_pelatihan->nama }} dengan kehadiran penuh sesuai dengan jadwal yang sudah ditetapkan oleh pelaksana kegiatan dan dibebastugaskan sementara oleh pimpinan agar dapat mengikuti pelatihan secara bersungguh-sungguh.
